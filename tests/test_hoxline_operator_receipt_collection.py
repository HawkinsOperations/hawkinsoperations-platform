...
class HoxlineOperatorReceiptCollectionTests(unittest.TestCase):
    ...
    def test_collector_accepts_ho_det_009_rule_family_child_rule(self) -> None:
        attestation = ho_factory.hoxline_operator_attestation_sample()
        attestation_path = self.write_json("operator-attestation-009-child", attestation)
        execution_id = ho_factory.hoxline_sanitized_live_receipt_sample("HO-DET-009")["execution_id"]
        record = ho_factory.hoxline_operator_wazuh_alert_sample("HO-DET-009", execution_id=execution_id)
        record["rule"]["id"] = "910092"
        packet_path = str(Path(tempfile.gettempdir()) / "ho-det-009-child-rule-packet.json")

        result = ho_factory.hoxline_collect_operator_receipt_from_wazuh(
            detection_id="HO-DET-009",
            execution_id=execution_id,
            alerts_json=self.write_json("ho-det-009-child-rule-alerts", record),
            time_window_start_utc=attestation["source_time_window_start_utc"],
            time_window_end_utc=attestation["source_time_window_end_utc"],
            operator_attestation_path=attestation_path,
            output_path=packet_path,
    ...
    def test_collector_rejects_unrelated_ho_det_009_rule(self) -> None:
        attestation = ho_factory.hoxline_operator_attestation_sample()
        attestation_path = self.write_json("operator-attestation-009-unrelated", attestation)
        execution_id = ho_factory.hoxline_sanitized_live_receipt_sample("HO-DET-009")["execution_id"]
        record = ho_factory.hoxline_operator_wazuh_alert_sample("HO-DET-009", execution_id=execution_id)
        ...
        with self.assertRaises(ho_factory.FactoryError):
            ho_factory.hoxline_collect_operator_receipt_from_wazuh(
                detection_id="HO-DET-009",
                execution_id=execution_id,
                alerts_json=self.write_json("ho-det-009-unrelated-rule-alerts", record),
                time_window_start_utc=attestation["source_time_window_start_utc"],
                time_window_end_utc=attestation["source_time_window_end_utc"],
                operator_attestation_path=attestation_path,
                output_path=str(Path(tempfile.gettempdir()) / "ho-det-009-unrelated-rule-packet.json"),
            )
