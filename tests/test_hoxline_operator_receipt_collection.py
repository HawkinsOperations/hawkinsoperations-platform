...
class HoxlineOperatorReceiptCollectionTests(unittest.TestCase):
    ...
    def test_collector_accepts_account_rule_family_child_rules(self) -> None:
        for detection_id, child_rule_id, label in (("HO-DET-009", "910092", "009"), ("HO-DET-010", "910102", "010")):
            with self.subTest(detection_id=detection_id):
                attestation = ho_factory.hoxline_operator_attestation_sample()
                attestation_path = self.write_json(f"operator-attestation-{label}-child", attestation)
                execution_id = ho_factory.hoxline_sanitized_live_receipt_sample(detection_id)["execution_id"]
                record = ho_factory.hoxline_operator_wazuh_alert_sample(detection_id, execution_id=execution_id)
                record["rule"]["id"] = child_rule_id
                packet_path = str(Path(tempfile.gettempdir()) / f"ho-det-{label}-child-rule-packet.json")

                result = ho_factory.hoxline_collect_operator_receipt_from_wazuh(
                    detection_id=detection_id,
                    execution_id=execution_id,
                    alerts_json=self.write_json(f"ho-det-{label}-child-rule-alerts", record),
                    time_window_start_utc=attestation["source_time_window_start_utc"],
                    time_window_end_utc=attestation["source_time_window_end_utc"],
                    operator_attestation_path=attestation_path,
                    output_path=packet_path,
    ...
    def test_collector_rejects_unrelated_account_rule_families(self) -> None:
        for detection_id, unrelated_rule_id, label in (("HO-DET-009", "100204", "009"), ("HO-DET-010", "910011", "010")):
            with self.subTest(detection_id=detection_id):
                attestation = ho_factory.hoxline_operator_attestation_sample()
                attestation_path = self.write_json(f"operator-attestation-{label}-unrelated", attestation)
                execution_id = ho_factory.hoxline_sanitized_live_receipt_sample(detection_id)["execution_id"]
                record = ho_factory.hoxline_operator_wazuh_alert_sample(detection_id, execution_id=execution_id)
                record["rule"]["id"] = unrelated_rule_id

                with self.assertRaises(ho_factory.FactoryError):
                    ho_factory.hoxline_collect_operator_receipt_from_wazuh(
                        detection_id=detection_id,
                        execution_id=execution_id,
                        alerts_json=self.write_json(f"ho-det-{label}-unrelated-rule-alerts", record),
                        time_window_start_utc=attestation["source_time_window_start_utc"],
                        time_window_end_utc=attestation["source_time_window_end_utc"],
                        operator_attestation_path=attestation_path,
                        output_path=str(Path(tempfile.gettempdir()) / f"ho-det-{label}-unrelated-rule-packet.json"),
                    )
